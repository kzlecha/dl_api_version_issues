...
# All Libraries saved in JSON Format, see full list in documentation
deprecation = {
    "tensorflow":
        {
...
def filter_deprecated_libraries(df):
    '''
    ...
    # iterate through every datapoint and append if necessary
    for i in df.index:
        row = df.loc[i]

        # check the libraries for the appropriate point
        for key in deprecation.keys():
            if key in row["tags"].lower():
                # get the question timeline
                q_date = datetime.fromisoformat(row["question_date"])
                for release_date in deprecation[key]:
                    # if the release is older than the question - break
                    # this is because releases are in chronological order
                    if release_date > q_date:
                        break
                    for deprecated_api in deprecation[key][release_date]:
                        if deprecated_api.lower() in row["question_body"].lower():
                            df_libs.append(row.values)
                            source.append("question")
                            break
                        if row["answer_body"] is not None and deprecated_api.lower() in row["answer_body"].lower():
                            df_libs.append(row.values)
                            source.append("answer")
                            break
    
    df_libs = DataFrame(df_libs, columns=df.columns)
    df_libs['source'] = source
...
if __name__ == "__main__":
    ...
    filter_deprecated_libraries(df).to_csv("data/filtered/deprecated_libraries.csv", index=False)
